...
const Pin = ({ pin: { postedBy, image, _id, destination, save } }) => {
  ...
  // prettier-ignore
  const alreadySaved = !!(save?.filter(item => item.postedBy?._id === userInfo?.uid))?.length; //throughs undefined add ?
          ...
          <div className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pt-2 pb-2 z-50">
            <div className="flex justify-between items-center">
              ...
              {alreadySaved ? (
                <button
                  type="button"
                  className="bg-teal-600 opacity-70 hover:opacity-100 text-white font-bold px-5 py-1 text-base rounded-3xl shadow-md outline-none"
                >
                  {save?.length} Saved
                </button>
              ) : (
            ...
            <div className="flex justify-between items-center gap-2 w-full">
              ...
              {postedBy?._id === userInfo?.uid && (
                <button
                  ...
                  className="bg-white p-2 opacity-70 hover:opacity-100 bg-teal-600 font-bold  
